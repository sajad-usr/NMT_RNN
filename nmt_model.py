...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        enc_hiddens, last_ = self.encoder(nn.utils.rnn.pack_padded_sequence(X, source_lengths))  # enc_hiddens: (src_len, b, h*2)
        ...
        last_hidden, last_cell = last_[0], last_[1]
        last_hidden = torch.concat((last_hidden[0], last_hidden[1]), dim=1)
        last_cell = torch.concat((last_cell[0], last_cell[1]), dim=1)
        dec_hidden = self.h_projection(last_hidden)
        dec_cell = self.c_projection(last_cell)
